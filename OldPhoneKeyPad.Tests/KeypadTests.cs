...
namespace OldPhoneKeyPad.Tests
{
    public class KeypadTests
    {
        ...
        [Fact]
        public void WeirdOrNonPrintable_Characters_DoNotThrow()
        {
            var input = "\0\uFFFF\u0007abc!@#漢字";
            var ex = Record.Exception(() => KeypadDecoder.OldPhonePad(input));
            Assert.Null(ex);
        ...
        {
            // '2' pending then '*' cancels -> nothing emitted
        ...
        {
            // 2 (pending) -> ' ' flush => 'A' appended,
            // 2 (pending) -> '*' cancels pending,
            // '*' when no pending removes last emitted -> removes 'A'
        ...
        public void Star_Removes_Last_Emitted_When_No_Pending()
        {
            ...
            // '2' then ' ' -> flush -> emits 'A'
            // '*' now with no pending -> removes last emitted ('A')
            // '#' stops processing
        ...
        // ---------------------------
        // Cycle behavior tests (one test per key to cover cycles and digit emission)
        // ---------------------------
        ...
        public void Key7_Cycling_Behaviour()
        {
            ...
            Assert.Equal("P", KeypadDecoder.OldPhonePad("777777#")); // wrap
        }
